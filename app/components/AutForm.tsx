...
const AutForm = ({ type }: { type:FormType}) => {
  ...
  return (
    ...
    <div className="card-border lg:min-w-[566px]">
      <div className="flex flex-col gap-6 card py-10 px-10">
        <div className="flex flex-row gap-2 justify-center">
          <Image src="/logo.svg" alt="logo" width={38} height={30} />
          <h2>Hire ready</h2>
        </div>
        <h3>
          Hire in job  interviews with Hire Ready
        </h3>
      </div>
      
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className=" w-full space-y-6 mt-6 form px-4">
    ...
    <p className="text-center">
      {isSignIn? 'No account yet?': 'Hvae account already?'}
